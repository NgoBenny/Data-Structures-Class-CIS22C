   ...
   public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt)
    {
        ...
        for (int i = 0; i < rhyme.getLength(); i++) {
            playing = startAt % players.getLength();

            if (playing == 0) 
                playing = players.getLength();

            System.out.println("Player " + players.getEntry(playing) + ": " + rhyme.getEntry(i+1));
            startAt++;
        }
        System.out.println("Removing Player " + players.getEntry(playing));
        players.remove(playing);

        doRhyme(players, rhyme, playing);
